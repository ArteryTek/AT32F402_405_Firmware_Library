...
/** @addtogroup 405_USB_device_two_otg_device_demo USB_device_two_otg_device_demo
  * @{
  */
...
/* enable otg clock */
  ...
  /* init usb */
  usbd_init(&otghs_core_struct,
            USBHS_SPEED_CORE_ID,
            USBHS_ID,
            &cdc_class_handler,
            &cdc_desc_handler);
            
  usbd_init(&otgfs_core_struct,
            USBFS_SPEED_CORE_ID,
            USBFS_ID,
  ...
  while(1)
  {
    ...
    data_len = usb_vcp_get_rxdata(&otghs_core_struct.dev, usb_buffer);
    ...
    if(data_len > 0 || send_zero_packet == 1)
    {
      ...
      */
      ...
      {
        /* send data to host */
        if(usb_vcp_send_data(&otghs_core_struct.dev, usb_buffer, data_len) == SUCCESS)
        {
          break;
        }
      }while(timeout --);
    ...
    if(button_press == 1)
    {
      ...
      custom_hid_class_send_report(&otgfs_core_struct.dev, report_buf, USBD_CUSTOM_IN_MAXPACKET_SIZE);
      button_press = 0;
    }
  }
}
...
/**
  * @brief  this function handles otg interrupt.
  * @param  none
  * @retval none
  */
void OTGHS_IRQ_HANDLER(void)
{
  usbd_irq_handler(&otghs_core_struct);
}
...
{
  usbd_irq_handler(&otgfs_core_struct);
}
