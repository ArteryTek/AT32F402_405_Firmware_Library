...
/** @addtogroup 405_QSPI_command_port_using_dma_and_pem
  * @{
  */
...
/**
  * @brief  esmt32m cmd read pem begin config
  * @param  qspi_cmd_struct: the pointer for qspi_cmd_type parameter
  * @param  addr: read start address
  * @param  counter: read data counter
  * @retval none
  */
void esmt32m_cmd_read_pem_begin_config(qspi_cmd_type *qspi_cmd_struct, uint32_t addr, uint32_t counter)
{
  qspi_cmd_struct->pe_mode_enable = TRUE;
  qspi_cmd_struct->pe_mode_operate_code = PEMOPC_SET;
...
/**
  * @brief  esmt32m cmd read pem keep config
  * @param  qspi_cmd_struct: the pointer for qspi_cmd_type parameter
  * @param  addr: read start address
  * @param  counter: read data counter
  * @retval none
  */
void esmt32m_cmd_read_pem_keep_config(qspi_cmd_type *qspi_cmd_struct, uint32_t addr, uint32_t counter)
{
  qspi_cmd_struct->pe_mode_enable = TRUE;
  qspi_cmd_struct->pe_mode_operate_code = PEMOPC_SET;
  qspi_cmd_struct->instruction_code = 0xEB;
  qspi_cmd_struct->instruction_length = QSPI_CMD_INSLEN_0_BYTE;
  qspi_cmd_struct->address_code = addr;
  qspi_cmd_struct->address_length = QSPI_CMD_ADRLEN_3_BYTE;
  qspi_cmd_struct->data_counter = counter;
  qspi_cmd_struct->second_dummy_cycle_num = 4;
...
/**
  * @brief  esmt32m cmd read pem end config
  * @param  qspi_cmd_struct: the pointer for qspi_cmd_type parameter
  * @param  addr: read start address
  * @param  counter: read data counter
  * @retval none
  */
void esmt32m_cmd_read_pem_end_config(qspi_cmd_type *qspi_cmd_struct, uint32_t addr, uint32_t counter)
{
  qspi_cmd_struct->pe_mode_enable = TRUE;
  qspi_cmd_struct->pe_mode_operate_code = PEMOPC_RESET;
  qspi_cmd_struct->instruction_code = 0xEB;
  qspi_cmd_struct->instruction_length = QSPI_CMD_INSLEN_0_BYTE;
  qspi_cmd_struct->address_code = addr;
  qspi_cmd_struct->address_length = QSPI_CMD_ADRLEN_3_BYTE;
...
void qspi_read_data_using_pem(pem_step_type pem_step, uint32_t addr, uint32_t total_len, uint8_t* buf)
{
  ...
  {
    esmt32m_cmd_read_pem_begin_config(&esmt32m_cmd_config, addr, total_len);
    qspi_cmd_operation_kick(QSPI1, &esmt32m_cmd_config);
  }
  else if(pem_step == PEM_STEP_KEEP)
  {
    esmt32m_cmd_read_pem_keep_config(&esmt32m_cmd_config, addr, total_len);
    qspi_cmd_operation_kick(QSPI1, &esmt32m_cmd_config);
  }
  else if(pem_step == PEM_STEP_END)
  {
    esmt32m_cmd_read_pem_end_config(&esmt32m_cmd_config, addr, total_len);
    qspi_cmd_operation_kick(QSPI1, &esmt32m_cmd_config);
  }
