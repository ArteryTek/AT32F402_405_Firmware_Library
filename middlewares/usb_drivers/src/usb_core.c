...
/** @addtogroup AT32F402_405_middlewares_usb_drivers
  * @{
  */
...
#endif
  otgdev->cfg.dma_en = FALSE;
  
#ifdef OTG_USE_DMA
  otgdev->cfg.dma_en = TRUE;
#endif
                  ...
                  usbd_desc_handler *desc_handler)
{
  ...
  if(otgdev->cfg.dma_en)
  {
    otgdev->dev.dma_en = TRUE;
  }
  else
  {
    otgdev->dev.dma_en = FALSE;
  }
                  ...
                  usbh_user_handler_type *user_handler)
{
  ...
  if(otgdev->cfg.dma_en)
  {
    otgdev->host.dma_en = TRUE;
  }
  else
  {
    otgdev->host.dma_en = FALSE;
  }
