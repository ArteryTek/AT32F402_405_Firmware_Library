...
/** @addtogroup AT32F402_405_middlewares_usbh_drivers
  * @{
  */
...
usb_sts_type usbh_in_out_request(usbh_core_type *uhost, uint8_t hc_num)
{
  ...
  if(uhost->speed == USB_HIGH_SPEED)
  {
    ...
    {
      ch->hcintmsk_bit.ackmsk = FALSE;
      ch->hcintmsk_bit.nakmsk = FALSE;
      ch->hcintmsk_bit.nyetmsk = FALSE;
    }
    if((uhost->dma_en == FALSE) && (uhost->hch[hc_num].do_ping == TRUE))
    {
      usb_do_ping(usbx, hc_num);
      return USB_OK;
    }
  }
#endif
  ...
  {
    ch->hcdma = (uint32_t)uhost->hch[hc_num].trans_buf;
  }
#endif
  /* set odd frame */
                            ...
                            uint8_t core_id)
{
  ...
  if(uhost->dma_en == TRUE && hs_phytype)
  {
