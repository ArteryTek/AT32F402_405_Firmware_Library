...
/** @addtogroup AT32F402_405_middlewares_usbd_drivers
  * @{
  */
...
{
  /* setup parse */
  usbd_setup_request_parse(&udev->setup, (uint8_t *)udev->setup_buffer);
...
#ifdef OTG_USE_DMA
/**
  * @brief  endpoint 0 out dma set
  * @param  udev: to the structure of usbd_core_type
  * @retval none
  */
void usbd_ept0_out_dma_set(usbd_core_type *udev)
{
  otg_eptout_type *usb_outept = USB_OUTEPT(udev->usb_reg, 0);
  
  if(usb_outept->doepctl_bit.eptena == TRUE)
  {
    return;
  }
  USB_OUTEPT(udev->usb_reg, 0)->doepdma = (uint32_t)udev->setup_buffer;
  USB_OUTEPT(udev->usb_reg, 0)->doepctl_bit.usbacept = TRUE;
  USB_OUTEPT(udev->usb_reg, 0)->doepctl_bit.eptena = TRUE;
}
#endif
/**
...
void usbd_ept_open(usbd_core_type *udev, uint8_t ept_addr, uint8_t ept_type, uint16_t maxpacket)
{
  ...
  usb_ept_open(usbx, ept_info);
    ...
    if(((ept_addr & 0x7F) == 1) && (udev->speed == USB_HIGH_SPEED))
    {
      if(ept_info->inout == EPT_DIR_IN)
      {
        OTG_DEVICE(udev->usb_reg)->deachintmsk_bit.inep1intmsk = TRUE;
        OTG_DEVICE(usbx)->daintmsk &= ~(1 << ept_info->eptn);
      }
      else
      {
        OTG_DEVICE(udev->usb_reg)->deachintmsk_bit.outep1intmsk = TRUE;
                            ...
                            uint8_t core_id)
{
  ...
  if((core_id == USB_HIGH_SPEED_CORE_ID) && hs_phytype)
  {
  ...
  if(udev->dma_en == TRUE && hs_phytype)
  {
  ...
  else
  {
    if(hs_phytype == 0xC0 )
    {
      dev->dcfg_bit.devspd = 1; /*full speed in high*/
    }
    else
    {
  ...
  usbx->gintmsk = USB_OTG_SOF_INT | 
                  ...
                  USB_OTG_OTGINT_INT;
  if(udev->dma_en == FALSE || hs_phytype == 0)
  {
