...
static usb_sts_type class_init_handler(void *udev)
{
  ...
  {
    pcdcmsc->maxpacket = USBD_HS_MAXPACKET_SIZE;
    
    /* open in endpoint */
    usbd_ept_open(pudev, USBD_CDC_BULK_IN_EPT, EPT_BULK_TYPE, USBD_HS_CDC_MSC_IN_MAXPACKET_SIZE);
  
    /* open out endpoint */
    usbd_ept_open(pudev, USBD_CDC_BULK_OUT_EPT, EPT_BULK_TYPE, USBD_HS_CDC_MSC_OUT_MAXPACKET_SIZE);
    
    /* open in endpoint */
    usbd_ept_open(pudev, USBD_MSC_BULK_IN_EPT, EPT_BULK_TYPE, USBD_HS_CDC_MSC_IN_MAXPACKET_SIZE);
  
    /* open out endpoint */
    usbd_ept_open(pudev, USBD_MSC_BULK_OUT_EPT, EPT_BULK_TYPE, USBD_HS_CDC_MSC_OUT_MAXPACKET_SIZE);
  }
  else
  {
    pcdcmsc->maxpacket = USBD_FS_MAXPACKET_SIZE;
  ...
  usbd_ept_recv(pudev, USBD_CDC_BULK_OUT_EPT, pcdcmsc->g_rx_buff, pcdcmsc->maxpacket);
...
static usb_sts_type cdc_struct_init(cdc_msc_struct_type *pcdc)
{
  ...
  pcdc->g_rx_buff = (uint8_t *)g_cdc_rx_buffer;
  pcdc->g_cmd = (uint8_t *)g_cdc_cmd_buffer;
  return USB_OK;
}
...
uint16_t usb_vcp_get_rxdata(void *udev, uint8_t *recv_data)
{
 ...
 if(pcdc->g_rx_completed == 0)
  ...
  usbd_ept_recv(pudev, USBD_CDC_BULK_OUT_EPT, pcdc->g_rx_buff, pcdcmsc->maxpacket);
