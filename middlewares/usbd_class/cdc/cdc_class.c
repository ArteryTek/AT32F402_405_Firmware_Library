...
static usb_sts_type class_init_handler(void *udev)
{
  ...
  {
    pcdc->maxpacket = USBD_HS_CDC_MAXPACKET_SIZE;
    
    /* open out endpoint */
    usbd_ept_open(pudev, USBD_CDC_BULK_OUT_EPT, EPT_BULK_TYPE, USBD_HS_CDC_OUT_MAXPACKET_SIZE);

    /* open in endpoint */
    usbd_ept_open(pudev, USBD_CDC_BULK_IN_EPT, EPT_BULK_TYPE, USBD_HS_CDC_IN_MAXPACKET_SIZE);
  }
  else
  {
    pcdc->maxpacket = USBD_FS_CDC_MAXPACKET_SIZE;
  ...
  if(pudev->speed == USB_HIGH_SPEED)
  {
    ...
    usbd_ept_recv(pudev, USBD_CDC_BULK_OUT_EPT, pcdc->g_rx_buff, USBD_HS_CDC_OUT_MAXPACKET_SIZE);
  }
  else
  {
    /* set out endpoint to receive status */
...
uint16_t usb_vcp_get_rxdata(void *udev, uint8_t *recv_data)
{
  ...
  usbd_ept_recv(pudev, USBD_CDC_BULK_OUT_EPT, pcdc->g_rx_buff, pcdc->maxpacket);
