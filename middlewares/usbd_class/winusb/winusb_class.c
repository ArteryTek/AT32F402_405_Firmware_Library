...
static uint32_t g_winusb_rx_buffer[128];
...
static usb_sts_type class_init_handler(void *udev)
{
  ...
  {
    p_winusb->maxpacket = USBD_HS_WINUSB_MAXPACKET_SIZE;
    
    /* open out endpoint */
    usbd_ept_open(pudev, USBD_WINUSB_BULK_OUT_EPT, EPT_BULK_TYPE, USBD_HS_WINUSB_OUT_MAXPACKET_SIZE);

    /* open in endpoint */
    usbd_ept_open(pudev, USBD_WINUSB_BULK_IN_EPT, EPT_BULK_TYPE, USBD_HS_WINUSB_IN_MAXPACKET_SIZE);
  }
  else
  {
  ...
  if(pudev->speed == USB_HIGH_SPEED)
  {
    ...
    usbd_ept_recv(pudev, USBD_WINUSB_BULK_OUT_EPT, p_winusb->g_rx_buff, USBD_HS_WINUSB_OUT_MAXPACKET_SIZE);
  }
  else
  {
    /* set out endpoint to receive status */
