...
ALIGNED_HEAD static uint8_t g_usbd_configuration[USBD_VCPKYBRD_CONFIG_DESC_SIZE] ALIGNED_TAIL =
{
  ...
  #pragma data_alignment=4
#endif
ALIGNED_HEAD static uint8_t g_usbd_hs_configuration[USBD_VCPKYBRD_CONFIG_DESC_SIZE] ALIGNED_TAIL =
{
  USB_DEVICE_CFG_DESC_LEN,               /* bLength: configuration descriptor size */
  USB_DESCIPTOR_TYPE_CONFIGURATION,      /* bDescriptorType: configuration */
  LBYTE(USBD_VCPKYBRD_CONFIG_DESC_SIZE), /* wTotalLength: bytes returned */
  ...
  USBD_VCPKYBRD_CDC_BULK_IN_EPT,         /* bEndpointAddress: the address of endpoint on usb device described by this descriptor */
  USB_EPT_DESC_BULK,                     /* bmAttributes: endpoint attributes */
  LBYTE(USBD_HS_VCPKYBRD_IN_MAXPACKET_SIZE),
  HBYTE(USBD_HS_VCPKYBRD_IN_MAXPACKET_SIZE), /* wMaxPacketSize: maximum packe size this endpoint */
  0x00,                                  /* bInterval: interval for polling endpoint for data transfers */
  ...
  USBD_VCPKYBRD_CDC_BULK_OUT_EPT,        /* bEndpointAddress: the address of endpoint on usb device described by this descriptor */
  USB_EPT_DESC_BULK,                     /* bmAttributes: endpoint attributes */
  LBYTE(USBD_HS_VCPKYBRD_OUT_MAXPACKET_SIZE),
  HBYTE(USBD_HS_VCPKYBRD_OUT_MAXPACKET_SIZE),/* wMaxPacketSize: maximum packe size this endpoint */
  0x00,                                  /* bInterval: interval for polling endpoint for data transfers */
  ...
  LBYTE(USBD_VCPKYBRD_IN_MAXPACKET_SIZE),
  HBYTE(USBD_VCPKYBRD_IN_MAXPACKET_SIZE),/* wMaxPacketSize: maximum packe size this endpoint */
  VCPKYBRD_HS_HID_BINTERVAL_TIME,           /* bInterval: interval for polling endpoint for data transfers */
};

ALIGNED_HEAD static uint8_t g_usbd_other_speed[USBD_VCPKYBRD_CONFIG_DESC_SIZE] ALIGNED_TAIL =
{
  USB_DEVICE_CFG_DESC_LEN,               /* bLength: configuration descriptor size */
  USB_DESCIPTOR_TYPE_CONFIGURATION,      /* bDescriptorType: configuration */
  LBYTE(USBD_VCPKYBRD_CONFIG_DESC_SIZE), /* wTotalLength: bytes returned */
...
/* hs config descriptor */
static usbd_desc_t config_descriptor_hs =
{
  USBD_VCPKYBRD_CONFIG_DESC_SIZE,
  g_usbd_hs_configuration
};

/* config othe speed */
static usbd_desc_t config_other_speed =
{
  USBD_VCPKYBRD_CONFIG_DESC_SIZE,
  g_usbd_other_speed
};
...
static usbd_desc_t device_qualifier =
{
  USB_DEVICE_QUALIFIER_DESC_LEN,
  g_usbd_qualifier,
};
...
/**
  * @brief  get hs config descriptor
  * @param  none
  * @retval usbd_desc                         
  */
static usbd_desc_t *get_hs_device_configuration(void)
