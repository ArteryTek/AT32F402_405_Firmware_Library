...
static usb_sts_type class_init_handler(void *udev)
{
  ...
  if(pudev->speed == USB_HIGH_SPEED)
  {
    ...
    /* open in endpoint */
    usbd_ept_open(pudev, USBD_VCPKYBRD_CDC_BULK_IN_EPT, EPT_BULK_TYPE, USBD_HS_VCPKYBRD_IN_MAXPACKET_SIZE);

    /* open out endpoint */
    usbd_ept_open(pudev, USBD_VCPKYBRD_CDC_BULK_OUT_EPT, EPT_BULK_TYPE, USBD_HS_VCPKYBRD_OUT_MAXPACKET_SIZE);
  }
  else
  {
    vcpkybrd->maxpacket = USBD_FS_VCPKYBRD_MAXPACKET_SIZE;
...
uint16_t usb_vcpkybrd_vcp_get_rxdata(void *udev, uint8_t *recv_data)
{
  ...
  usbd_ept_recv(pudev, USBD_VCPKYBRD_CDC_BULK_OUT_EPT, vcpkybrd->g_rx_buff, vcpkybrd->maxpacket);
