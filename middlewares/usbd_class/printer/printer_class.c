...
/*printer receive buffer define*/
static uint32_t g_printer_rx_buffer[128];
static uint32_t g_printer_cmd_buffer[16];
...
static usb_sts_type class_init_handler(void *udev)
{
  ...
  pprter->g_rx_buff = (uint8_t *)g_printer_rx_buffer;
  pprter->g_printer_data = (uint8_t *)g_printer_cmd_buffer;
  
  if(pudev->speed == USB_HIGH_SPEED)
  {
    pprter->maxpacket = USBD_HS_PRINTER_MAXPACKET_SIZE;
    
    /* open in endpoint */
    usbd_ept_open(pudev, USBD_PRINTER_BULK_IN_EPT, EPT_BULK_TYPE, USBD_HS_PRINTER_IN_MAXPACKET_SIZE);

    /* open out endpoint */
    usbd_ept_open(pudev, USBD_PRINTER_BULK_OUT_EPT, EPT_BULK_TYPE, USBD_HS_PRINTER_OUT_MAXPACKET_SIZE);
  }
  else
  {
    pprter->maxpacket = USBD_PRINTER_MAXPACKET_SIZE;
  ...
  usbd_ept_recv(pudev, USBD_PRINTER_BULK_OUT_EPT, pprter->g_rx_buff, pprter->maxpacket);
